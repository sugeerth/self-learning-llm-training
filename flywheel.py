...
def run_generations(rounds: int, steps: int, samples_per_round: int,
                    ...
                    sample_len: int = 64) -> dict:
    """Multi-generation self-training: G0 trains on real data; each Gk+1
    trains on real + ALL accumulated filtered synthetic data generated by Gk.
    Same torch seed across a chain -> only the data differs. The question the
    single-shot A/B can't answer: does self-training compound or degrade?
    ...
    with ProcessPoolExecutor(max_workers=profile.workers, mp_context=ctx,
        ...
        for g in range(rounds + 1):
            ...
            if g == rounds:
                break
            # each seed's model extends its own corpus
            for s in range(seeds):
                ...
                old = np.fromfile(corpus[s], dtype=np.uint16)
                ...
                for smp in kept:
                    ...
                    synth.append(10)
                np.concatenate([old, np.asarray(synth, dtype=np.uint16)]).tofile(nxt)
                corpus[s] = nxt
    ...
    report = {"rounds": rounds, "seeds": seeds, "steps_per_generation": steps,
              ...
              "ratio_per_round": ratio_per_round,
              "generations": drift, "filter_stats": filter_stats,
              "verdict": verdict}
    path = REPORT.replace(".json", "_generations.json")
