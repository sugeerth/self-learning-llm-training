...
def ppl_gate(samples: list[list[int]], losses: list[float],
             drop_worst: float = 0.2, drop_best: float = 0.05) -> tuple[list[list[int]], dict]:
    """Drop samples the generator itself finds implausible (high loss ->
    likely garbage) AND the ones it finds suspiciously easy (lowest loss ->
    likely memorized/looping text the n-gram gates missed). Pure logic —
    losses are computed by the caller."""
    if not samples:
        return [], {"ppl_gate_kept": 0}
    order = sorted(range(len(samples)), key=lambda i: losses[i])
    lo = int(len(samples) * drop_best)
    hi = len(samples) - int(len(samples) * drop_worst)
    keep_idx = set(order[lo:hi])
    kept = [s for i, s in enumerate(samples) if i in keep_idx]
    return kept, {"ppl_gate_in": len(samples), "ppl_gate_kept": len(kept),
                  "loss_band": [round(losses[order[lo]], 3),
                                round(losses[order[hi - 1]], 3)] if kept else None}


def score_samples(ckpt_path: str, samples: list[list[int]],
                  batch: int = 32) -> list[float]:
    """Mean next-token loss of each sample under the generator checkpoint."""
    import torch
    ...
    with torch.no_grad():
        for i in range(0, len(samples), batch):
            ...
            width = min(len(s) for s in chunk)
            x = torch.tensor([s[:width] for s in chunk], device=device)
            logits, _ = model(x[:, :-1])
            per_tok = F.cross_entropy(
                logits.reshape(-1, logits.size(-1)), x[:, 1:].reshape(-1),
                reduction="none").view(x.size(0), -1)
            losses.extend(per_tok.mean(dim=1).tolist())
    return losses
...
# ────────────────────── iterated generations (collapse curve) ──────────────────────

def run_generations(rounds: int, steps: int, samples_per_round: int,
                    ratio_per_round: float, real_tokens: int, seeds: int,
                    sample_len: int = 64, mode: str = "accumulate",
                    use_ppl_gate: bool = False) -> dict:
    """Multi-generation self-training: G0 trains on real data; each Gk+1
    trains on real + filtered synthetic data generated by Gk. Same torch seed
    across a chain -> only the data differs. The question the single-shot A/B
    can't answer: does self-training compound or degrade?

    mode='accumulate': synthetic data piles up (share grows each round).
    mode='replace': each round's synthetic REPLACES the last (share constant)
    — our measured drift tracked the growing share, so this isolates it.
    use_ppl_gate: additionally drop samples the generator scores as garbage
    (worst 20%) or as suspicious loops (best 5%).

    Verdict from the drift of deterministic real-val loss vs G0, per seed.
    """
    from concurrent.futures import ProcessPoolExecutor
    from multiprocessing import get_context
    ...
    profile = load_or_tune(quick=True)
    run_dir = os.path.join(RUNS_DIR, f"flywheel-gen-{int(time.time())}")
    os.makedirs(run_dir, exist_ok=True)
    train_bin, _ = prepare()
    real = np.array(np.memmap(train_bin, dtype=np.uint16, mode="r")[:real_tokens])

    # per-seed corpus starts as the same real subset
    corpus = {}
    for s in range(seeds):
        corpus[s] = os.path.join(run_dir, f"s{s}_g0.bin")
        real.tofile(corpus[s])

    base = {"cfg": GEN_CFG, "steps": steps, "lr": 3e-4,
            "batch_size": 8, "val_batches": 8, "ckpt_in": None,
            "want_sample": False, "deterministic_val": True}
    losses: dict[int, list[float]] = {s: [] for s in range(seeds)}
    ...
    with ProcessPoolExecutor(max_workers=profile.workers, mp_context=ctx,
        ...
        for g in range(rounds + 1):
            t0 = time.time()
            cks = {s: os.path.join(run_dir, f"s{s}_g{g}.pt") for s in range(seeds)}
            tasks = [dict(base, train_bin=corpus[s], ckpt_out=cks[s],
                          torch_seed=2000 + s) for s in range(seeds)]
            evals = list(pool.map(_train_task, tasks))
            for s, ev in enumerate(evals):
                losses[s].append(ev["val_loss"])
            print(f"flywheel-gen: G{g} val_loss="
                  f"{[round(ev['val_loss'], 3) for ev in evals]}"
                  f" ({time.time() - t0:.0f}s)")

            if g == rounds:
                break
            # each seed's model extends (or refreshes) its own corpus
            for s in range(seeds):
                raw = generate_samples(cks[s], samples_per_round, sample_len,
                                       prefix_len=16, seed=100 * s + g)
                kept, fstats = heuristic_filter(raw)
                if use_ppl_gate and kept:
                    kept, gstats = ppl_gate(kept, score_samples(cks[s], kept))
                    fstats.update(gstats)
                filter_stats.append({"seed": s, "generation": g, **fstats})
                base_tokens = (np.fromfile(corpus[s], dtype=np.uint16)
                               if mode == "accumulate" else real)
                nxt = os.path.join(run_dir, f"s{s}_g{g + 1}.bin")
                cap = int(real_tokens * ratio_per_round)
                synth: list[int] = []
                for smp in kept:
                    if len(synth) >= cap:
                        break
                    synth.extend(smp[: cap - len(synth)])
                    synth.append(10)
                np.concatenate([base_tokens,
                                np.asarray(synth, dtype=np.uint16)]).tofile(nxt)
                corpus[s] = nxt

    # drift verdicts (paired vs each chain's own G0)
    drift = []
    for g in range(rounds + 1):
        deltas = [losses[s][g] - losses[s][0] for s in range(seeds)]
        rel = (sum(deltas) / len(deltas)) / (sum(l[0] for l in losses.values()) / seeds)
        drift.append({"generation": g,
                      "val_loss_per_seed": [round(losses[s][g], 4) for s in range(seeds)],
                      "drift_vs_g0": f"{rel * 100:+.2f}%"})
    final_rel = float(drift[-1]["drift_vs_g0"].rstrip("%"))
    verdict = ("gain" if final_rel < -2 else "collapse" if final_rel > 2 else "stable")

    report = {"rounds": rounds, "seeds": seeds, "steps_per_generation": steps,
              "samples_per_round": samples_per_round,
              "ratio_per_round": ratio_per_round, "mode": mode,
              "ppl_gate": use_ppl_gate,
              "generations": drift, "filter_stats": filter_stats,
              "verdict": verdict}
    path = REPORT.replace(".json", f"_generations_{mode}.json")
    with open(path, "w") as f:
        json.dump(report, f, indent=2)
    print(f"\nflywheel-gen: verdict after {rounds} self-training generations: "
          f"{verdict.upper()} (final drift {drift[-1]['drift_vs_g0']})")
    for d in drift:
        print(f"  G{d['generation']}: loss={d['val_loss_per_seed']} drift={d['drift_vs_g0']}")
    print(f"flywheel-gen: wrote {path}")
    return report
...
def main() -> None:
    ...
    g = sub.add_parser("generations",
                       help="iterated self-training: collapse-or-compound curve")
    g.add_argument("--rounds", type=int, default=3)
    g.add_argument("--seeds", type=int, default=2)
    g.add_argument("--steps", type=int, default=64, help="training steps per generation")
    g.add_argument("--samples", type=int, default=192, help="samples generated per round")
    g.add_argument("--ratio", type=float, default=0.1,
                   help="max synthetic added per round, as fraction of real")
    g.add_argument("--mode", choices=["accumulate", "replace"], default="accumulate")
    g.add_argument("--ppl-gate", action="store_true",
                   help="also drop generator-scored garbage/loop samples")
    args = ap.parse_args()
    ...
    if args.cmd == "run":
        ...
        run_generations(rounds=args.rounds, steps=args.steps,
                        samples_per_round=args.samples,
                        ratio_per_round=args.ratio, real_tokens=120_000,
                        seeds=args.seeds, mode=args.mode,
                        use_ppl_gate=args.ppl_gate)
